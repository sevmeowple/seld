...
#            pool.join()
    ...
    def extract_all_labels(self):
        ...
        for fold_cnt, sub_folder in enumerate(os.listdir(self._desc_dir)):
            loc_desc_folder = os.path.join(self._desc_dir, sub_folder) #ty:ignore[no-matching-overload]
        ...
        for _frame_ind in range(_sed_labels.shape[0]):
            ...
            if len(_tmp_ind[0]):
                ...
                for _tmp_class in _tmp_ind[0]:
                    if _is_polar:
                        _output_dict[_frame_ind].append([_tmp_class, _azi_labels[_frame_ind, _tmp_class], _ele_labels[_frame_ind, _tmp_class]]) #ty:ignore[not-subscriptable]
                    else:
                        _output_dict[_frame_ind].append([_tmp_class, _x[_frame_ind, _tmp_class], _y[_frame_ind, _tmp_class], _z[_frame_ind, _tmp_class]])#ty:ignore[not-subscriptable]
        return _output_dict
