...
def get_params(argv="1"):
    ...
    params = dict(
        ...
        # 修改为您自己的路径：
        dataset_dir="/disk7/zchan/Dataset/Dcase2023",
        # OUTPUT PATHS
        # feat_label_dir='DCASE2020_SELD_dataset/feat_label_hnet/',  # Directory to dump extracted features and labels
        # 同时也建议修改输出路径，确保您有权限写入：
        feat_label_dir="/disk7/zchan/Dataset/Dcase2023/feat_label",
        # 特征存放目录
    ...
    feature_label_resolution = int(params["label_hop_len_s"] // params["hop_len_s"])  # ty:ignore
    params["feature_sequence_length"] = (
        params["label_sequence_length"] * feature_label_resolution
    )
    params["t_pool_size"] = [feature_label_resolution, 1, 1]  # CNN time pooling
    params["patience"] = int(params["nb_epochs"])  # ty:ignore
